#include <stdio.h>
#include <stdlib.h>
/* AVL Tree Node */ struct Node {
int key;
struct Node *left; struct Node *right; int height;
};
/* Utility function to get maximum */ int max(int a, int b) {
return (a > b) ? a : b;
}
/* Get height of the tree */ int height(struct Node *N) {
if (N == NULL)
return 0; return N->height;
}
/* Create a new node */
struct Node* newNode(int key) { struct Node* node = (struct Node*)
malloc(sizeof(struct Node)); node->key = key;
node->left = NULL; node->right = NULL; node->height = 1; return node;
}
/* Right rotate */
struct Node* rightRotate(struct Node* y) { struct Node* x = y->left;
struct Node* T2 = x->right; x->right = y;
y->left = T2;
y->height = max(height(y->left), height(y->right)) + 1; x->height = max(height(x->left), height(x->right)) + 1; return x;
}
/* Left rotate */
struct Node* leftRotate(struct Node* x) { struct Node* y = x->right;
struct Node* T2 = y->left; y->left = x;
x->right = T2;
x->height = max(height(x->left), height(x->right)) + 1; y->height = max(height(y->left), height(y->right)) + 1; return y;
 
}
/* Get balance factor */
int getBalance(struct Node* N) { if (N == NULL)
return 0;
return height(N->left) - height(N->right);
}
/* Insert into AVL Tree */
struct Node* insert(struct Node* node, int key) { if (node == NULL)
...
if ((root->left == NULL) || (root->right == NULL)) { struct Node* temp = root->left ? root->left : root->right; if (temp == NULL) {
temp = root; root = NULL;
} else
*root = *temp; free(temp);
} else {
...
root->height = 1 + max(height(root->left), height(root->right)); int balance = getBalance(root);
/* LL */
...
case 3:
printf("Enter key to search: "); scanf("%d", &key); search(root, key);
break; case 4:
